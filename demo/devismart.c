...
{ 
  ...
  int packetSize = header->dataSize + sizeof(struct MsgHeader);
  const char *cmd;
  ...
  case MDG_ADD_PAIRING:
    ...
    mdg_chat_output_fprintf("%s ", cmd);
    chatclient_hexdump(&payload[1], payload[0]);
	break;
  ...
  default:
    ...
    if (cmd)
    {
      mdg_chat_output_fprintf("%s %d ", cmd, header->dataSize);
	  data += sizeof(struct MsgHeader);
	  size = header->dataSize;
	}
	else
	{
      mdg_chat_output_fprintf("Unknown command code %d in packet:\n", header->msgCode);
    }
	chatclient_hexdump(data, size);
  }
...
/*
 * This seems to be a workaround to some protocol bug (or feature ???)
 * The first byte on a connection stream must be thrown away. Original
 * DeviSmart app implements the same logic.
 */
static char newConnections[1024];

void devismart_connection_begin(uint32_t connection_id)
{
  if (connection_id < sizeof(newConnections))
    newConnections[connection_id] = 1;
  else
	mdg_chat_output_fprintf("Connection id %u is too large to be handled correctly!\n", connection_id);
}

uint32_t devismart_receive_data(const uint8_t *data, uint32_t size, uint32_t connection_id)
{
  if ((connection_id < sizeof(newConnections)) && newConnections[connection_id])
  {
	newConnections[connection_id] = 0;
    data++;
	size--;
  }
  ...
  while (size >= sizeof(struct MsgHeader))
  {
	...
	if (handled == -1)
	{
	  mdg_chat_output_fprintf("Malformed stream at position %d; size exceeds maximum:\n", data - start);
      chatclient_hexdump(start, origSize);
	  return 0;
	}
  ...
  if (size)
  {
    mdg_chat_output_fprintf("Leftover fragment; size %d:\n", size);
	chatclient_hexdump(data, size);
  }
